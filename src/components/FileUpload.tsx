...
const FileUpload: React.FC = () => {
	...
	const handleUpload = async () => {
		try {
			...
			let completeUploadPayload: {
				...
				wrappedKeys?: { publicKey: string; encryptedKey: string }[];
			} = {
				token: results.token,
	...
	return (
		<>
			<Header />
			<div
				className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 relative overflow-hidden pt-20"
												...
												<div className="mb-3 flex justify-end">
													{receiverMode ? (
														<button
															...
															className={`inline-flex items-center justify-center gap-1.5 text-xs px-3 py-1.5 rounded-full border text-p4 cursor-pointer hover:scale-105 transition-all duration-200 ${
														...
														<button
															onClick={handleModeSwitch}
															className="inline-flex items-center justify-center gap-1.5 text-xs px-3 py-1.5 rounded-full border border-yellow-500/40 bg-yellow-500/10 text-p4 cursor-pointer hover:scale-105 transition-all duration-200 hover:bg-yellow-500/20"
														>
									...
									<button
										onClick={() => {
											if (receiverMode) {
												...
												// If no mode selected, go back to dashboard
												navigate('/dashboard');
											}
										}}
										className="text-p4/70 hover:text-p4 transition-colors"
									>
										<X size={20} />
									</button>
								</div>
